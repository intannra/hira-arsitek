...
if ( ! function_exists( 'gmr_setup' ) ) :
	...
	function gmr_setup() {
		...
		register_nav_menus( array(
			...
			'secondary' => esc_html__( 'Secondary', 'superfast' )
		) );
...
/*
if( !function_exists( 'baru_ad_match_content' ) ) {
    function baru_ad_match_content() {
		$banner = '';
		$banner .= '<!-- Banner Setelah Related Post -->';
		return $banner;
	}
}
if ( !function_exists( 'baru_ad_tambah' ) ) :
	function baru_ad_tambah( $content ) {
		if( is_single() && in_the_loop() ) {
				$content = $content . baru_ad_match_content();

		}
		return $content;
	}
endif; // endif baru_ad_tambah
add_filter( 'the_content', 'baru_ad_tambah', 42 ); 

remove_filter( 'the_content', 'idblog_core_add_banner_after_content',30 );
add_filter( 'the_content', 'idblog_core_add_banner_after_content', 0 );
