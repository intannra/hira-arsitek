...
class Envira_Gallery_Shortcode {
	...
	public function __construct() {
		...
		wp_register_script( $this->base->plugin_slug . '-script', plugins_url( 'assets/js/min/envira-min.js', $this->base->file ), array( 'jquery', 'jquery-masonry' ), $this->base->version, true );
				...
				var responsivelyLazy = window.responsivelyLazy;
					...
					if ( $this->get_config( 'lightbox_enabled', $data ) ) {
						...
						envira_gallery_options = {
							...
							padding: <?php echo in_array( $this->get_config( 'lightbox_theme', $data ), array( 'base_dark' ), true ) ? '4' : '15'; ?>,
							cyclic: true,
							...
							arrows: <?php echo in_array( $this->get_config( 'lightbox_theme', $data ), array( 'base_dark' ), true ) ? 'true' : esc_attr( $this->get_config( 'arrows', $data ) ); ?>,
							...
							helpers: {
								...
								thumbs: {
									...
									<?php if ( in_array( $this->get_config( 'lightbox_theme', $data ), array( 'base_dark' ), true ) ) : ?>
									dynamicMargin: true,
									<?php endif; ?>
									position: '
									<?php
									echo in_array( $this->get_config( 'lightbox_theme', $data ), array( 'base_dark' ), true )
													? 'bottom'
								...
								<?php endif; ?>
								<?php if ( $this->get_config( 'toolbar', $data ) && ! in_array( $this->get_config( 'lightbox_theme', $data ), array( 'base_dark' ), true ) ) : ?>
								buttons: {
	...
	public function load_gallery_theme( $theme ) {
		...
		foreach ( $this->common->get_gallery_themes() as $array => $data ) {
			...
			if ( file_exists( plugin_dir_path( $data['file'] ) . 'themes/' . $theme . '/style.css' ) ) {
				...
				wp_enqueue_style( $this->base->plugin_slug . $theme . '-theme', plugins_url( 'themes/' . $theme . '/css/style.css', $data['file'] ), array( $this->base->plugin_slug . '-style' ), $this->base->version );
			}
			break;
		}
